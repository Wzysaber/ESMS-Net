...
def validating(args, num_classes, model, optimizer, train_dataset, val_loader, device, epoch):
    ...
    model_name = '%d:  OA=%.4f miou=%.4f F1=%.4f' % (epoch, val_acc, val_mean_IoU, val_F1_score)

    # 保存相应训练中最好的模型
    if val_mean_IoU > args.best_miou:
        if os.path.exists(os.path.join(args.directory, args.Delmodel_name + '.pth')):
            os.remove(os.path.join(args.directory, args.Delmodel_name + '.pth'))

        if args.save_file:
            torch.save(model.state_dict(), os.path.join(args.directory, model_name + '.pth'))

        args.Delmodel_name = model_name

        args.best_miou = val_mean_IoU
        args.best_OA = val_acc
        args.best_F1 = val_F1_score
    ...
    print('best_OA = %.2f%%  best_miou = %.2f%%   best_F1 = %.2f%%' % (
        args.best_OA * 100, args.best_miou * 100, args.best_F1 * 100))
