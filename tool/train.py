...
def data_set(args):
    ...
    # 是否使用数据增强
    if args.sync_transform:
        data_Augmentation = sync_transform
    else:
        data_Augmentation = None

    # 数据集的载入和相应参数
    train_dataset = RSDataset(root=args.train_data_root, mode='src', sync_transforms=data_Augmentation)  # 加载数据集
...
def training(args, num_classes, model, optimizer, train_dataset, train_loader, criterion1, criterion2, device, epoch):
    ...
    if epoch < 200:
        ...
        max_iter = args.total_epochs * len(train_loader)
        curr_iter = (epoch-199) * len(train_loader)  # 训练的数量
        lr = (args.base_lr * 0.1) * (1 - float(curr_iter) / max_iter) ** 0.9
    ...
    for index, data in enumerate(tbar):
        ...
        dice_loss2 = criterion2(outputs, masks, softmax=True)

        loss = 0.5 * loss1 + 0.5 * dice_loss2
