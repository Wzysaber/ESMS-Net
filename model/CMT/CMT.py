...
class Decoder_block(nn.Module):
    ...
    def forward(self, x):
        ...
        up_4_conv = self.stage_up_4(dilateformerX4)
        dilateformerX4_up = self.upsample_4(up_4_conv)

        up_3_conv = torch.cat([dilateformerX4_up, dilateformerX3], dim=1)
        up_3_conv = self.stage_up_3(up_3_conv)
        dilateformerX3_up = self.upsample_3(up_3_conv)

        up_2_conv = torch.cat([dilateformerX3_up, dilateformerX2], dim=1)
        up_2_conv = self.stage_up_2(up_2_conv)
        dilateformerX2_up = self.upsample_2(up_2_conv)

        up_1_conv = torch.cat([dilateformerX2_up, dilateformerX1], dim=1)
...
class Mlp(nn.Module):
    ...
    def forward(self, x, H, W):
        B, N, C = x.shape
        x = x.permute(0, 2, 1).reshape(B, C, H, W)  # 还原回卷积的格式
        ...
        x = self.conv2(x)
        x = x.flatten(2).permute(0, 2, 1)  # 再打回1D序列格式
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W, relative_pos):
        ...
        x = self.proj(cnn_feat) + cnn_feat  # CNN部分卷积

        x = x.flatten(2).permute(0, 2, 1)  # 还原回相应的1D序列
